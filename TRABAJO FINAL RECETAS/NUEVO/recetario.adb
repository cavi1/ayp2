...
-- cargar archivo
   ...
   procedure Cargar_archivo (recetario: in out Trecetario) is
      --R:Trecetamaster;
      Archi: Archivo_Recetas.File_Type;
      Rta: Character;
   begin
      Put_line(" quiere cargar el archivo, ponga 's' ");
      Create (Archi, Out_File, "recetas.txt");
      New_Line;
      Get(Rta);skip_line;
      if Rta='s' then
         --Open(Archi, Out_File, "recetas.txt");
         while not Vacia(Recetario) loop
            Write (Archi, Info(Recetario));
...
-- bajar archivo
   ...
   procedure Bajar_Archivo (Recetario: in out Trecetario) is
      Archi: Archivo_Recetas.File_Type;
      Recetamasteraux: Trecetamaster;
   begin
      Open(Archi, In_File, "recetas.txt");
      while not End_Of_File (Archi) loop
         Read(Archi, Recetamasteraux);
  ...
  begin
      ...
      while not Vacia(Recetario) and not encontrado loop
         if Nombre_Receta (1..Long_nombre) = Info_Nombre_Receta(info(Recetario).receta) (1..Long_nombre) then
           Encontrado:= True;
         end if;
   ...
   procedure Agregar_Receta (Recetario: in out Trecetario) is
      ...
      Nombre_Aux: Cade_Nombre_Receta;
      Long_Aux: Positive;
      Encontrado: Boolean;
   begin
      Creareceta(Receta);
      Nombre_Aux:= Info_Nombre_Receta (Receta);
      Long_aux:= info_long_nombre_receta(receta);
      Buscar_receta (Recetario_Aux, Nombre_Aux, Long_aux, Encontrado);
      if Encontrado then
         Put(" Ya existe una receta con el mismo nombre :v ");
      else
         Recetamasteraux.Receta:= Receta;
         Recetamasteraux.Aperturas:=0;
         Insertar(Recetario, Recetamasteraux);
         --Put (" quiere agregar esta receta al archivo? ");
         --Cargar_Receta(Recetamasteraux);
      end if;
   end Agregar_Receta;
   ...
   begin
      while not Vacia(Recetario_Aux) loop        
         Put(Info_Nombre_Receta(info(recetario_aux).receta)(1..info_long_nombre_receta(info(recetario_aux).receta)));
         New_Line;
   ...
   begin
      ...
      if Encontrado then
         Desplegareceta(Info(Recetario_Aux).Receta);         Recetamasteraux:= Info(Recetario_Aux);
         Suprimir (Recetario, Info(Recetario_Aux));
         Recetamasteraux.Aperturas:= Recetamasteraux.Aperturas +1;
         Insertar (Recetario, Recetamasteraux);
      else
         raise Receta_Inexistente;
      end if;
